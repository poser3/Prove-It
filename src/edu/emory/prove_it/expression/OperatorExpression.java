...
public class OperatorExpression extends Expression {
	...
	public OperatorExpression getParentOfSelected() {	
		if (this.isSelected()) {
			...
			for (int i=0; i<this.getNumArgs(); i++) {
				if (this.getArg(i) instanceof OperatorExpression) {
					OperatorExpression possibleParent = ((OperatorExpression) this.getArg(i)).getParentOfSelected();
	...
	public OperatorExpression getGrandParentOfSelected() {
		...
		else if (this.isGrandParentOfSelected()) {
			return this;
		}
		else {
			for (int i=0; i<this.getNumArgs(); i++) {
				if (this.getArg(i) instanceof OperatorExpression) {
					OperatorExpression possibleGrandParent = ((OperatorExpression) this.getArg(i)).getGrandParentOfSelected();
					if (possibleGrandParent != null) {
						return possibleGrandParent;
					}
				}
			}
		}
	...
	public boolean isParentOfSelected() {
		for (int i=0; i<this.getNumArgs(); i++) {
			if (this.getArg(i).isSelected()) {
				return true;
			}
		}
		//if you get this far, selected wasn't there to be found
		return false;
	}
	
	public boolean hasArg(Expression arg) {
		for (int i=0; i<this.getNumArgs(); i++) {
			if (this.getArg(i).equals(arg)) {
				return true;
			}
		}
		//if you get this far, arg wasn't there to be found
		return false;
	}
	...
	public boolean isGrandParentOfSelected() {
		for (int i=0; i<this.getNumArgs(); i++) {
			if (this.getArg(i) instanceof OperatorExpression) {
				if (((OperatorExpression) this.getArg(i)).isParentOfSelected()) {
					return true;
				}
			}
		}
		//if you get this far, it wasn't there to begin with
		return false;
	}
